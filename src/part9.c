...
// We will search in a leaf node with binary search
...
// In the lsat part (part 8) we are storing the keys in unsorted way, we always insert the new row at the end of the table.
// So now we will fix this, and we will detect and reject duplicate key, for that we will search in the table for the correct place,
// then insert there. If the key already exists there, return an error.